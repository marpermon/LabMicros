...
#define BUFFER_SIZE EI_CLASSIFIER_SLICE_SIZE // Size of one classification window
...
int main(void)
{
    ...
    while (1) {
        ...
        int16_t sensor_data[3] = {x,y,z}; // Replace with actual sensor functions

        // Add new data to the buffer
        for (size_t i = 0; i < SAMPLE_STRIDE; i++) {
            raw_buffer[buffer_offset++] = (float)sensor_data[i];
        }
